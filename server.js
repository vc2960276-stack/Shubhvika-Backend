...
function formatShopifyProduct(product) {
  ...
  const colorOption = options.find(
    (opt) => String(opt?.name || "").trim().toLowerCase() === "color"
  );
...
// ---------------------------------------------------------------------------
// CORS & Preflight Handling (Cross-origin credentials safe)
// ---------------------------------------------------------------------------

app.use((req, res, next) => {
  const origin = req.headers.origin;

  // Echo the exact request origin to fulfill credential mode policy
  if (origin) {
  ...
  // Instantly resolve browser CORS preflight check
...
app.get("/", (_req, res) => {
  res.json({ service: "SHUBHVIKA API", status: "ok", runtime: "node" });
});
...
api.get("/products", async (req, res) => {
  try {
    ...
    if (req.query.product_type) {
      const requestedProductType = String(req.query.product_type).trim().toLowerCase();
      formattedProducts = formattedProducts.filter(
        (product) =>
          String(product.product_type || "").trim().toLowerCase() === requestedProductType
      );
    }

    if (req.query.is_new !== undefined) {
      const requestedIsNew = String(req.query.is_new).toLowerCase() === "true";
      formattedProducts = formattedProducts.filter(
    ...
    if (req.query.is_popular !== undefined) {
      const requestedIsPopular = String(req.query.is_popular).toLowerCase() === "true";
      formattedProducts = formattedProducts.filter(
...
api.get("/auth/me", requireAuth, (req, res) => {
  ...
  res.json({
    user: { id, email, name, role: role || "customer" },
  });
});
...
api.post("/newsletter", async (req, res) => {
  try {
    ...
    if (!isValidEmail(email)) {
      return res.status(400).json({ detail: "Please enter a valid email." });
    }
...
// ---------------------------------------------------------------------------
// Mount on BOTH "/" and "/api" to eliminate Vercel path-stripping 404s
// ---------------------------------------------------------------------------
...
if (require.main === module) {
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[shubhvika] API running on http://localhost:${PORT}`);
  });
}